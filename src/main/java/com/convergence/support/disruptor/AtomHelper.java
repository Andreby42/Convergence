...
public class AtomHelper {
    ...
    public AtomHelper() {
        disruptor = new Disruptor<AtomEvent>(new AtomEventFactory(), BUFFER_SIZE,
                DaemonThreadFactory.INSTANCE, ProducerType.SINGLE, new YieldingWaitStrategy());
        // disruptor.handleEventsWith(new AtomEventHandler());
        disruptor.handleEventsWithWorkerPool(new AtomEventHandler("woker-1"),
                new AtomEventHandler("woker-2"), new AtomEventHandler("woker-3"));
